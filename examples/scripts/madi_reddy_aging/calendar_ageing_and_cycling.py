...
VARIANTS = {
    ...
    "Madi-Reddy": ({"SEI": "madi-reddy", "loss of active material": "stress-driven"}, 1.0),
}
...
def get_parameter_values(name, scale_factor):
    # Use OKane2022 as the base for all variants
    parameter_values = pybamm.ParameterValues("OKane2022")

    if name == "Madi-Reddy":
        # Synchronize with Madi-Reddy paper constants
        # a = 3e6. OKane2022 uses a = 3e5 roughly. 
        # Match a = 3*eps_s/R -> R = 3*eps_s/3e6
...
def run_ageing_phase(name, options, scale_factor, storage_days, initial_soc, logger):
    """Runs the initial calendar ageing (storage) phase."""
    logger.info(f"    Running initial {storage_days}-day storage ageing phase...")
    model = pybamm.lithium_ion.DFN(options)
    parameter_values = get_parameter_values(name, scale_factor)
...
def run_simulations(storage_days, initial_soc, total_cycles, chunk_size, logger, only_variant=None):
    ...
    for name, (options, scale_factor) in VARIANTS.items():
        ...
        aged_solution, storage_trace = run_ageing_phase(
            name, options, scale_factor, storage_days, initial_soc, logger
        )
