...
def run_continuous_hybrid():
    ...
    # 2. Original Paper Parameters with 50nm Initial Thickness
    ...
    param.update({
        ...
        "Initial SEI thickness [m]": 50e-9,  # Changed from 5e-9 to 50e-9 (50nm)
    ...
    # 4. Solve (Starting at baseline 90% SoC)
    print(f"Starting continuous hybrid experiment (50nm Initial SEI) with storage steps: {durations}...")
    ...
    sol = sim.solve(initial_soc=0.9, calc_esoh=False)
    ...
    plt.suptitle("Continuous Hybrid SEI Aging Experiment (50nm Initial SEI)")
    plt.tight_layout()
    plt.savefig("continuous_hybrid_50nm_experiment.png")
    print("\nExperiment complete. Results saved to continuous_hybrid_50nm_experiment.png")
