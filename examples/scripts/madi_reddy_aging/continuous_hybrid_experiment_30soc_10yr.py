...
def run_continuous_hybrid():
    ...
    param.update({
        ...
        "Initial SEI thickness [m]": 5e-9,  # Baseline 5nm thickness
    ...
    # Initial 10-year Phase: (10 years * 365 days * 24 hours = 87600 hours)
    # Sampling set to 6 months (6 * 30.416 * 24 hours ≈ 4380 hours)
    ...
    print(f"Starting continuous experiment (10yr precondition, 30% SoC start)...")
    ...
    for i, var in enumerate(variables):
        axes[i].plot(time_h, sol[var].entries, color="#264653", linewidth=1.5)
        axes[i].set_ylabel(var)
        axes[i].grid(True, alpha=0.3)
        
        # Add a thick line for the 10-year mark
        temp_time = 87600
        axes[i].axvline(temp_time, color="#e9c46a", linestyle="-", linewidth=2.0)
    ...
    plt.suptitle("Aging Experiment: 10yr Pre-Storage & Multi-Stage Cycling at 30% SoC (5nm Initial SEI)")
    plt.tight_layout()
    plt.savefig("continuous_hybrid_30soc_10yr.png")
    print("\nExperiment complete. Results saved to continuous_hybrid_30soc_10yr.png")
