...
def run_continuous_hybrid():
    ...
    # Storage durations from previous request: 7, 14, 21, 30, 90.
    ...
    # 4. Solve
    print(f"Starting continuous hybrid experiment with storage steps: {durations}...")
    sim = pb.Simulation(model, parameter_values=param, var_pts=var_pts, experiment=experiment,solver=pb.CasadiSolver(mode="safe",atol=1e-6, rtol=1e-6))   
    # We use initial_soc=0.9 to start at the requested SoC
    ...
    for i, var in enumerate(variables):
        axes[i].plot(time_h, sol[var].entries, color="#1d3557", linewidth=1.5)
        axes[i].set_ylabel(var)
        axes[i].grid(True, alpha=0.3)
        
        # Add vertical lines for each storage start
        temp_time = 0
        for days in durations:
            axes[i].axvline(temp_time, color="#e63946", linestyle="--", alpha=0.4)
            # Rough estimate of time: storage + 3 cycles (approx 20h per cycle)
            temp_time += days*24 + 3*20 

    axes[-1].set_xlabel("Time [h]")
    plt.suptitle("Continuous Hybrid SEI Aging Experiment (Storage -> 3 Cycles -> Repeat)")
    plt.tight_layout()
    plt.savefig("continuous_hybrid_experiment.png")
    print("\nExperiment complete. Results saved to continuous_hybrid_experiment.png")
